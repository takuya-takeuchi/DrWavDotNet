class Config
{
   ...
   @{
      "DrWavDotNet.Native"     = "DrWavDotNetNative.dll";
   }
   
   static $BuildLibraryLinuxHash = 
   @{
      "DrWavDotNet.Native"     = "libDrWavDotNetNative.so";
   }
   
   static $BuildLibraryOSXHash = 
   @{
      "DrWavDotNet.Native"     = "libDrWavDotNetNative.dylib";
   }
   
   static $BuildLibraryIOSHash = 
   @{
      "DrWavDotNet.Native"     = "libDrWavDotNetNative.a";
   }
   ...
   #***************************************
   # Arguments
   #  %1: Root directory of DrWavDotNet
   ...
   [string] GetNativeDrWavRootDir()
   {
      return   Join-Path $this.GetRootDir() src |
               Join-Path -ChildPath dr_libs
   }
   ...
   [string] GetStoreDriectory([string]$CMakefileDir)
   {
      ...
      return Join-Path $buildDir "DrWavDotNet" | `
...
function ConfigCPU([Config]$Config, [string]$CMakefileDir)
{
   ...
   # Build DrWav
   $installNativeDrWavDir = $Config.GetNativeDrWavRootDir().Replace("`\", "/")
   ...
   if ($IsWindows)
   {
      ...
      $env:drwav_DIR = $installNativeDrWavDir
      Write-Host "   cmake -G `"${VS}`" -A ${VSARC} -T host=x64 `
         -D drwav_DIR=`"${installNativeDrWavDir}`" `
...
function ConfigUWP([Config]$Config, [string]$CMakefileDir)
{
   if ($IsWindows)
   {
      ...
      $NativeDrWavDir = $Config.GetNativeDrWavRootDir().Replace("`\", "/")
      $patchFullPath = Join-Path $nugetDir $patch
      $current = Get-Location
      Set-Location -Path $NativeDrWavDir
      Write-Host "Apply ${patch} to ${NativeDrWavDir}" -ForegroundColor Yellow
...
function Reset-DrWav-Modification([Config]$Config, [string]$currentDir)
{
   $NativeDrWavDir = $Config.GetNativeDrWavRootDir()
   Set-Location -Path $NativeDrWavDir
   Write-Host "Reset modification of ${NativeDrWavDir}" -ForegroundColor Yellow
   git checkout .
...
function Build([Config]$Config)
{
   ...
   # revert DrWav
   Reset-DrWav-Modification $Config (Join-Path $Current $Output)
