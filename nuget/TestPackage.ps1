#***************************************
#Arguments
#%1: Test Package (DrWavDotNet.CUDA92)
...
function RunTest($BuildTargets)
{
   foreach($BuildTarget in $BuildTargets)
   {
      ...
      # Test
      $WorkDir = Join-Path $DrWavDotNet work
      $NugetDir = Join-Path $DrWavDotNet nuget
      ...
      $env:DrWavDotNet_VERSION = $VERSION
      $env:DrWavDotNet_GUI_SUPPORT = 1

      $TestDir = Join-Path $DrWavDotNet test | `
                 Join-Path -ChildPath DrWavDotNet.Tests

      $TargetDir = Join-Path $WorkDir DrWavDotNet.Tests
      ...
      dotnet remove reference "..\..\src\DrWavDotNet\DrWavDotNet.csproj" > $null
...
$BuildTargets = @()
$BuildTargets += New-Object PSObject -Property @{Target = "cpu";  Architecture = 64; CUDA = 0;   Package = "DrWavDotNet"         }
